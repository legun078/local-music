...
import {
  ...
  normalizeSlotUrl,
  packSlotForSave,
  readBangonPickerValue,
  renderDaySlotList,
  reorderSlotsInPart,
  resolveSlotDropPlacement,
  renderSlotStartTimePickerMarkup,
  syncBangonPickerControls,
...
let scheduleSaveFingerprint = null;

export function resetScheduleSaveBaseline() {
  if (!fullSchedule) {
    scheduleSaveFingerprint = null;
    return;
  }
  scheduleSaveFingerprint = JSON.stringify(buildSchedulePayload(fullSchedule));
}

function currentScheduleFingerprint() {
  if (!fullSchedule) return null;
  return JSON.stringify(buildSchedulePayload(fullSchedule));
}

function scheduleMonthsKey(payload) {
  return JSON.stringify((payload && payload.months) || {});
}

async function putSchedulePayload(payload, headers = {}) {
  const putHeaders = { ...headers };
  if (scheduleRevision != null) putHeaders["X-Schedule-Revision"] = String(scheduleRevision);
...
async function putScheduleWithConflictRecovery(payload) {
  let { res, body } = await putSchedulePayload(payload);
  if (res.status !== 409) return { res, body };

  if (body.revision != null) scheduleRevision = String(body.revision);
  const serverRes = await apiFetch("/api/schedule");
  if (!serverRes.ok) return { res, body };
  const serverSchedule = await serverRes.json().catch(() => null);
  if (!serverSchedule) return { res, body };

  const serverMonths = scheduleMonthsKey(buildSchedulePayload(serverSchedule));
  const ourMonths = scheduleMonthsKey(payload);
  if (serverMonths !== ourMonths) return { res, body };

  ({ res, body } = await putSchedulePayload(payload));
  return { res, body };
}

export async function syncScheduleRevision() {
  try {
    const base = window.SCHEDULE_BASE || "";
    const res = await fetch(`${base}/api/content-revisions`, {
      credentials: "same-origin",
      cache: "no-store",
    });
    if (!res.ok) return scheduleRevision;
    const body = await res.json();
    const raw = body?.revisions?.schedule;
    scheduleRevision = raw == null ? null : String(raw);
  } catch {
    /* 다음 저장 시 재시도 */
  }
...
export function applyScheduleRevision(revision) {
  if (revision != null) scheduleRevision = String(revision);
}
...
function coalesceDayParts(slots, bangonTime, part2) {
  ...
  let bangon = bangonTime || "";
    if (!part1Slots.length && !bangon && part2State) {
...
function packDayPayload(slots, bangonTime, part2) {
  ...
  // 2부는 1부(일정 또는 뱅온)가 있을 때만 보존
  if (normalized.length || time) {
...
export function setPartSlots(dateKeyStr, partIdx, slots, opts = {}) {
  ...
  const part2Bangon = norm.part2?.bangonTime || "";
  const part2Slots = slots || [];
  const part2 =
    part2Slots.length || part2Bangon
      ? { slots: part2Slots, bangonTime: part2Bangon }
      : null;
...
export function setPartBangonTime(dateKeyStr, partIdx, bangonTime) {
  ...
  if (!md) return;
  const norm = normalizeDay(md.days[dateKeyStr]) || {};
  if (!(norm.slots || []).length && !(norm.bangonTime || getBangonTime(md.days[dateKeyStr]))) return;
  const part2 = {
...
export function enableSecondPart(dateKeyStr) {
  ...
  if (!md) return false;
  const norm = normalizeDay(md.days[dateKeyStr]) || {};
  if (!(norm.slots || []).length && !(norm.bangonTime || getBangonTime(md.days[dateKeyStr])))
    return false;
...
export function moveSlot(fromDate, fromIdx, toDate, toIdx, fromPart = 0, toPart = fromPart, opts = {}) {
  const placement = opts.placement || (toIdx == null || toIdx < 0 ? "end" : "before");
  ...
  const slotText = String(fromSlots[fromIdx]?.text || "").trim();
  ...
  if (fromDate === toDate && fromPart === toPart) {
    const reordered = reorderSlotsInPart(fromSlots, fromIdx, toIdx, placement);
    setPartSlots(fromDate, fromPart, reordered, skipPrune);
    const fromMd = monthDataForDate(fromDate);
  ...
  const toSlots = [...getPartSlots(toDate, toPart)];
  let insertAt;
  if (toIdx == null || toIdx < 0 || placement === "end") {
    insertAt = toSlots.length;
  } else if (placement === "at") {
    insertAt = toIdx;
    if (fromIdx < toIdx) {
      insertAt = toIdx > fromIdx + 1 ? toIdx - 1 : toIdx;
    }
    insertAt = Math.min(insertAt, toSlots.length);
  } else {
    insertAt = toIdx;
...
export function saveScheduleNow(options) {
  clearTimeout(saveTimer);
  return saveSchedule(options);
}
...
async function saveSchedule({ silentIfUnchanged = false } = {}) {
  if (!fullSchedule || !canEdit) return { unchanged: true };
  if (saveInFlight) {
    saveQueued = true;
    return { queued: true };
  }
  ...
  try {
    ...
    const fingerprint = currentScheduleFingerprint();
    if (scheduleSaveFingerprint != null && fingerprint === scheduleSaveFingerprint) {
      return { unchanged: true };
    }
    if (!silentIfUnchanged) showSaveStatus("저장 중…");
    const payload = buildSchedulePayload(fullSchedule);
    const { res, body } = await putScheduleWithConflictRecovery(payload);
    if (res.status === 409) {
      throw new Error(
        body.hint || "다른 곳에서 일정이 변경되었습니다. 수정 모드를 꺼었다 켠 뒤 다시 시도하세요."
      );
    }
    if (!res.ok) {
      throw new Error(body.hint || body.error || `저장 실패 (${res.status})`);
    }
    if (body.revision != null) scheduleRevision = String(body.revision);
    scheduleSaveFingerprint = fingerprint;
    if (!body.unchanged && !silentIfUnchanged) showSaveStatus("저장됨", false);
    return { unchanged: Boolean(body.unchanged), ok: true };
  } catch (err) {
    if (!silentIfUnchanged) showSaveStatus(err.message || "저장 실패", true);
    throw err;
  } finally {
    saveInFlight = false;
    if (saveQueued) {
...
function onDrop(e) {
  ...
  if (targetChip && targetChip.dataset.date === toDate) {
    ...
    if (dragState.date === toDate && (dragState.part || 0) === toPart) {
      const drop = resolveSlotDropPlacement(dragState.index, toIndex);
      if (!drop) return;
      placement = drop.placement;
    } else {
      placement = "before";
    }
  } else if (targetList && partFromElement(targetList) === toPart) {
    placement = "end";
  }

  moveSlot(dragState.date, dragState.index, toDate, toIndex, dragState.part || 0, toPart, { placement });
  document.querySelectorAll(".drop-target").forEach((el) => el.classList.remove("drop-target"));
}
...
function onCtxAction(e) {
  ...
  if (action === "add" && ctxTarget.type === "day") {
    ...
    moveSlot(date, index, date, index - 1, part, part, { placement: "at" });
...
function closeSlotStartHourPanels(exceptPicker = null) {
  const panels = new Set([
    ...document.querySelectorAll(".slot-start-hour-panel"),
  ]);

  panels.forEach((panel) => {
    const hostPicker =
      panel.closest("[data-slot-start-picker]") ||
      panel._slotStartHourHost?.closest("[data-slot-start-picker]");
    if (exceptPicker && hostPicker === exceptPicker) return;
    panel.classList.add("hidden");
    resetSlotStartHourPanelLayout(panel);
    hostPicker
      ?.querySelector(".slot-start-hour-trigger")
      ?.setAttribute("aria-expanded", "false");
  });
}

const SLOT_START_HOUR_PANEL_MAX_HEIGHT = 320;
const SLOT_START_HOUR_PANEL_MIN_HEIGHT = 200;
const SLOT_START_HOUR_PANEL_GAP = 6;

function resetSlotStartHourPanelLayout(panel) {
  if (!panel) return;
  const host = panel._slotStartHourHost;
  if (host && panel.parentElement !== host) {
    host.appendChild(panel);
  }
  delete panel._slotStartHourHost;
  panel.classList.remove("slot-start-hour-panel--floating");
...
function scrollSlotStartHourPanelToActive(panel) {
  if (!panel) return;
  const active = panel.querySelector(".slot-start-hour-option.is-active");
  if (!active) return;
  const panelRect = panel.getBoundingClientRect();
  const activeRect = active.getBoundingClientRect();
  const delta =
    activeRect.top - panelRect.top - panel.clientHeight / 2 + activeRect.height / 2;
  panel.scrollTop = Math.max(0, panel.scrollTop + delta);
}

function layoutSlotStartHourPanel(trigger, panel) {
  if (!trigger || !panel) return;
  const margin = 12;
  const rect = trigger.getBoundingClientRect();
  const spaceBelow = window.innerHeight - rect.bottom - margin;
  const spaceAbove = rect.top - margin;
  const maxBelow = Math.min(SLOT_START_HOUR_PANEL_MAX_HEIGHT, Math.max(spaceBelow, 0));
  const maxAbove = Math.min(SLOT_START_HOUR_PANEL_MAX_HEIGHT, Math.max(spaceAbove, 0));
  const openUp = maxBelow < SLOT_START_HOUR_PANEL_MIN_HEIGHT && maxAbove > maxBelow;
  const maxHeight = Math.max(
    SLOT_START_HOUR_PANEL_MIN_HEIGHT,
    Math.min(SLOT_START_HOUR_PANEL_MAX_HEIGHT, openUp ? maxAbove : maxBelow)
  );

  /* contain/overflow 카드 밖으로 빼서 fixed가 뷰포트 기준으로 맞게 붙도록 */
  if (!panel._slotStartHourHost) {
    panel._slotStartHourHost = panel.parentElement;
  }
  if (panel.parentElement !== document.body) {
    document.body.appendChild(panel);
  }

  panel.classList.add("slot-start-hour-panel--floating");
  ...
  panel.style.top = openUp
    ? `${Math.max(margin, rect.top - maxHeight - SLOT_START_HOUR_PANEL_GAP)}px`
    : `${rect.bottom + SLOT_START_HOUR_PANEL_GAP}px`;
  panel.style.zIndex = "320";
}

function findSlotStartHourPanel(picker) {
  if (!picker) return null;
  const nested = picker.querySelector(".slot-start-hour-panel");
  if (nested) return nested;
  for (const panel of document.querySelectorAll(
    ".slot-start-hour-panel.slot-start-hour-panel--floating"
  )) {
    if (panel._slotStartHourHost?.closest("[data-slot-start-picker]") === picker) {
      return panel;
    }
  }
  return null;
...
function applySlotStartPickerValue(picker, value) {
  ...
  if (picker?.closest("#slot-edit-start-wrap")) {
  ...
  const hourPanel = findSlotStartHourPanel(picker) || picker;
  hourPanel.querySelectorAll(".slot-start-hour-option").forEach((btn) => {
  ...
  const bangonRoot = picker.closest("[data-bangon-picker]");
  if (bangonRoot) {
    syncBangonPickerControls(bangonRoot);
    // 패널이 body로 포탈된 뒤에도 방금 선택한 시각을 그대로 전달
    const bangonValue = readBangonPickerValue(bangonRoot) || (
      normalized
        ? `${normalized}${bangonRoot.querySelector(".bangon-approx-input")?.checked ? "~" : ""}`
        : ""
    );
    bangonRoot.dispatchEvent(
      new CustomEvent("bangontimechange", {
        bubbles: true,
        detail: { value: bangonValue },
      })
    );
  }
}
...
function readSlotStartPickerValue(picker) {
  if (!picker) return "";
  const hour = findActiveSlotStartHourOption(picker)?.dataset.hour ?? "";
  if (hour === "") return "";
...
function initSlotStartPickerControls() {
  if (document.documentElement.dataset.slotStartPickerBound) return;
  document.documentElement.dataset.slotStartPickerBound = "1";
  ...
  const onPickerClick = (e) => {
    ...
    if (trigger) {
      ...
      const panel = findSlotStartHourPanel(picker);
      ...
      if (willOpen) {
        layoutSlotStartHourPanel(trigger, panel);
        requestAnimationFrame(() => scrollSlotStartHourPanelToActive(panel));
      } else {
        resetSlotStartHourPanelLayout(panel);
      }
      return;
    ...
    if (hourBtn) {
      const panel = hourBtn.closest(".slot-start-hour-panel");
      const host = panel?._slotStartHourHost;
      const picker =
        hourBtn.closest("[data-slot-start-picker]") ||
        host?.closest("[data-slot-start-picker]");
      if (!picker) return;
  ...
  /* 뱅온·일정수정 공용 + body 포탈 패널 클릭 대응 */
  document.addEventListener("click", (e) => {
    if (
      e.target.closest(".slot-start-hour-trigger") ||
      e.target.closest(".slot-start-hour-option") ||
      e.target.closest(".slot-start-minute-option")
    ) {
      onPickerClick(e);
      return;
    }
    if (e.target.closest(".slot-start-hour-field")) return;
    if (e.target.closest(".slot-start-hour-panel")) return;
    closeSlotStartHourPanels();
  });
}
...
function onSlotEditSave() {
  ...
  if (index != null && prevSlots[index]) {
    const prevPacked = packSlotForSave(prevSlots[index]);
    const nextPacked = packSlotForSave(slot);
    if (JSON.stringify(prevPacked) === JSON.stringify(nextPacked)) {
      closeSlotEdit();
      return;
    }
  }
