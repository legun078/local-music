...
/** 캘린더·모달 표시용 (예: 오전 10시, 낮 12시, 밤 12시, 오후 4시 쯤) */
...
/** 시 옵션 패널이 body로 포탈된 경우에도 활성 시 버튼을 찾는다 */
export function findActiveSlotStartHourOption(startPicker) {
  if (!startPicker) return null;
  const nested = startPicker.querySelector(".slot-start-hour-panel .slot-start-hour-option.is-active");
  if (nested) return nested;
  for (const panel of document.querySelectorAll(".slot-start-hour-panel.slot-start-hour-panel--floating")) {
    const host = panel._slotStartHourHost;
    if (host && startPicker.contains(host)) {
      return panel.querySelector(".slot-start-hour-option.is-active");
    }
  }
  return null;
}
...
export function syncBangonPickerControls(picker) {
  if (!picker) return;
  const startPicker = picker.querySelector("[data-slot-start-picker]") || picker;
  const hourBtn = findActiveSlotStartHourOption(startPicker);
  const hasHour = Boolean(hourBtn && (hourBtn.dataset.hour ?? "") !== "");
  const approxInput = picker.querySelector(".bangon-approx-input");
  const approxToggle = picker.querySelector(".bangon-approx-toggle");
  startPicker.querySelectorAll(".slot-start-minute-option").forEach((btn) => {
  ...
  startPicker.classList.toggle("has-time", hasHour);
}

export function readBangonPickerValue(picker) {
  if (!picker) return "";
  const startPicker = picker.querySelector("[data-slot-start-picker]") || picker;
  const hour = findActiveSlotStartHourOption(startPicker)?.dataset.hour ?? "";
  if (hour === "") return "";
  const minute =
    startPicker.querySelector(".slot-start-minute-option.is-active")?.dataset.minute ?? "0";
...
/** 뱅온 시간 선택 UI (자세히 보기 상단) — 일정 시작 시간과 동일 커스텀 피커 */
export function renderBangonTimePickerMarkup(activeValue) {
  ...
  return `
    <div class="bangon-picker" data-bangon-picker>
      ...
      <div class="bangon-picker-time-row">
        ${renderSlotStartTimePickerMarkup(hasHour ? timeCore : "")}
        ${approxToggle}
      </div>
    </div>`;
}
...
export function normalizeDay(dayData, fallbackCategory = FALLBACK_CATEGORY) {
  ...
  // 1부가 완전히 비었을 때만 2부 승격 (뱅온만 있는 1부는 유지)
  if (!slots.length && !bangonTime && part2Raw) {
  ...
  // 2부: 1부에 일정 또는 뱅온이 있을 때만 유효
  const hasPart1 = slots.length > 0 || Boolean(bangonTime);
  const part2Slots = part2Raw && hasPart1 ? normalizeSlotItems(part2Raw.slots, dayCat) : [];
  const part2Bangon =
    part2Raw && hasPart1 && !isOffDaySlots(part2Slots)
      ? normalizeBangonValue(part2Raw.bangonTime)
      : "";
...
export function resolveSlotDropPlacement(fromIndex, overIndex) {
  if (fromIndex === overIndex) return null;
  return { overIndex, placement: "at" };
}

/** 같은 부 안에서 일정 순서를 바꾼다. overIndex는 기준 슬롯, placement는 at|before|after|end. */
export function reorderSlotsInPart(slots, fromIndex, overIndex, placement = "before") {
  if (!Array.isArray(slots) || !slots.length) return Array.isArray(slots) ? [...slots] : [];
  if (fromIndex < 0 || fromIndex >= slots.length) return [...slots];

  if (placement === "end" || overIndex == null || overIndex < 0) {
    const next = [...slots];
  ...
  if (placement === "at") {
    if (fromIndex < overIndex) {
      insertAt = overIndex > fromIndex + 1 ? overIndex - 1 : overIndex;
    }
...
export function slotOrderContext(slots, index, bangon = "") {
  return { slots, index, bangon: bangon || "" };
}

export function slotListHasEvening(slots, bangon = "") {
  return (slots || []).some((slot, index) => isEveningSlot(slot, slotOrderContext(slots, index, bangon)));
}

export function renderDaySlotList(normalized, categories, options = {}) {
  const slots = normalized?.slots || [];
  if (!slots.length) return "";
  const bangon = options.bangon ?? normalized?.bangonTime ?? "";
  const hasEvening = slotListHasEvening(slots, bangon);
  const listClass = `slot-list${hasEvening ? " slot-list--has-evening" : ""}`;

  const chips = slots
    .map((slot, index) => {
      const ctx = slotOrderContext(slots, index, bangon);
      return renderSlotChip(slot, categories, {
        ...options,
        slotIndex: index,
        evening: isEveningSlot(slot, ctx),
      });
    })
    .join("");
