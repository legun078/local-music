...
(() => {
  ...
  function renderNextDayItems(items) {
    ...
    const rows = list
      .map((it, i) => {
        const text = String(typeof it === "object" ? it?.text || "" : it || "").trim();
        if (!text) return "";
        const start = String(typeof it === "object" ? it?.startLabel || "" : "").trim();
        return `<li class="rpt__next-item" style="--i:${i}">
          <span class="rpt__next-item-text">${esc(text)}</span>
          ${start ? `<span class="rpt__next-item-time">${esc(start)}</span>` : ""}
        </li>`;
      })
      .filter(Boolean);
    if (!rows.length) return "";
    return `<ul class="rpt__next-items">${rows.join("")}</ul>`;
  }
        ...
        `<div class="rpt__next">
          ...
          ${parts
            .map((p) => {
              ...
              const bangon = formatBangonLabel(p.bangonLabel || p.timeLabel || "");
              const itemHtml = renderNextDayItems(items);
              return `<div class="rpt__next-part">
                <div class="rpt__next-head">
                  <b>${esc(part || p.headerLabel || "")}</b>
                  ${bangon ? `<span>${esc(bangon)}</span>` : ""}
                </div>
                ${
                  itemHtml ||
                  `<p class="rpt__next-empty">${esc(slide.emptyHint || "등록된 일정이 없어요")}</p>`
                }
              </div>`;
            })
            .join("")}
