(() => {
  ...
  function chartPlotContentWidth(plotBase, zoom = chartZoomLevel()) {
    const base = Math.max(180, Math.round(Number(plotBase) || CHART_W));
    const z = Math.max(CHART_ZOOM_MIN, Math.min(CHART_ZOOM_MAX, Number(zoom) || CHART_ZOOM_DEFAULT));
    if (z <= CHART_ZOOM_MIN + 0.001) return base;
    return Math.round(base * z);
  }

  function chartStageMetrics(rootEl, dual = true) {
    const root = rootEl || els.dataBody;
    const axisR = dual ? CHART_AXIS_R_DUAL : CHART_AXIS_R_SINGLE;
    const scroller = root?.querySelector?.("[data-chart-plot-scroll]");
    let plotBase = scroller?.clientWidth >= 180 ? scroller.clientWidth : 0;
    if (!plotBase) {
      const total = chartLayoutWidth(root);
      plotBase = Math.max(180, total - CHART_AXIS_L - axisR);
    }
    const plotW = chartPlotContentWidth(plotBase);
    return { plotBase, plotW, axisR };
  }

  function syncChartPlotScroll(root) {
    if (!root) return;
    root.querySelectorAll("[data-chart-plot-scroll]").forEach((scroller) => {
      if (chartIsFitZoom()) {
        scroller.scrollLeft = 0;
        return;
      }
  ...
  function scheduleChartPanFromPlotScroll(scroller) {
    ...
    chartPanScrollTimer = setTimeout(() => {
      ...
      try {
        sessionStorage.setItem(CHART_PAN_STORAGE_KEY, String(pan));
      } catch (_) {
        /* ignore */
      }
    }, 80);
  ...
  function scheduleChartPanFromWheel(scroller, deltaPx) {
  ...
  function onChartPanWheel(ev) {
    ...
    ev.preventDefault();
    scheduleChartPanFromWheel(scroller, dx);
  }

  function renderChartYAxisLeft(yTicks, yAt, h, axisW = CHART_AXIS_L) {
    return `<svg class="ending-dev-chart__axis-svg" viewBox="0 0 ${axisW} ${h}" width="${axisW}" height="${h}" aria-hidden="true">${yTicks
      .map((v) => {
        const y = yAt(v).toFixed(1);
        return `<line class="ending-dev-chart__grid ending-dev-chart__grid--axis" x1="${axisW - 5}" y1="${y}" x2="${axisW - 1}" y2="${y}" />
          <text class="ending-dev-chart__ylabel" x="${axisW - 8}" y="${y}" text-anchor="end" dominant-baseline="middle">${esc(fmtNum(v))}</text>`;
      })
      .join("")}</svg>`;
  }

  function renderChartYAxisRight(yTicks, yAt, h, axisW = CHART_AXIS_R_DUAL) {
    return `<svg class="ending-dev-chart__axis-svg" viewBox="0 0 ${axisW} ${h}" width="${axisW}" height="${h}" aria-hidden="true">${yTicks
      .map((v) => {
        const y = yAt(v).toFixed(1);
        return `<text class="ending-dev-chart__ylabel ending-dev-chart__ylabel--right" x="8" y="${y}" text-anchor="start" dominant-baseline="middle">${esc(fmtNum(v))}</text>`;
      })
      .join("")}</svg>`;
  }

  function renderChartPlotStage(opts) {
    const o = opts || {};
    const dual = Boolean(o.dual);
    const h = Number(o.h) || CHART_H;
    const plotW = Number(o.plotW) || CHART_W;
    const plotPad = o.plotPad || CHART_PLOT_PAD;
    const axisR = dual ? CHART_AXIS_R_DUAL : CHART_AXIS_R_SINGLE;
  ...
  function chartPeakNearTop(y, pad, h) {
    return y < pad.t + (h - pad.t - pad.b) * 0.42;
  }

  function chartPeakBoxOverlap(a, b, gap = 4) {
    return a.x0 < b.x1 + gap && a.x1 + gap > b.x0;
  }

  function renderPeakDot(m) {
    return `<circle class="ending-dev-chart__peak ${m.extraClass}" cx="${m.x.toFixed(1)}" cy="${m.y.toFixed(1)}" r="3.4" vector-effect="non-scaling-stroke" />`;
  }

  function renderPeakBadgeGroup(m, boxX, boxY, boxW, boxH, leadX1, leadY1, leadX2, leadY2) {
    const lx = boxX + boxW / 2;
    const ly = boxY + boxH - 3;
    const text = fmtNum(m.peak.v);
    return `<g class="ending-dev-chart__peak ${m.extraClass}" pointer-events="none">
      <line class="ending-dev-chart__peak-lead" x1="${leadX1.toFixed(1)}" y1="${leadY1.toFixed(1)}" x2="${leadX2.toFixed(1)}" y2="${leadY2.toFixed(1)}" vector-effect="non-scaling-stroke" />
      ${renderPeakDot(m)}
      <rect class="ending-dev-chart__peak-badge" x="${boxX.toFixed(1)}" y="${boxY.toFixed(1)}" width="${boxW.toFixed(1)}" height="${boxH.toFixed(1)}" rx="4" />
      <text x="${lx.toFixed(1)}" y="${ly.toFixed(1)}" text-anchor="middle">${esc(text)}</text>
    </g>`;
  }

  function renderPeakFlagMark(m, pad, w, h, occupied) {
    const text = fmtNum(m.peak.v);
    const boxW = Math.max(24, text.length * 6.2 + 10);
    const boxH = 13;
    const boxY = pad.t + 1;
    let boxX = m.x - boxW / 2;
    for (let i = 0; i < 8; i++) {
      ...
      if (!occupied.some((box) => chartPeakBoxOverlap(trial, box, 6))) break;
      boxX += boxW * 0.55;
    }
    boxX = Math.max(pad.l + 2, Math.min(w - pad.r - boxW - 2, boxX));
    occupied.push({ x0: boxX, x1: boxX + boxW, y0: boxY, y1: boxY + boxH });
    const lx = boxX + boxW / 2;
    const lyBottom = boxY + boxH;
    return renderPeakBadgeGroup(m, boxX, boxY, boxW, boxH, lx, lyBottom, m.x, m.y);
  }

  function renderPeakSideMark(m, pad, w, h, prefer, occupied) {
    const text = fmtNum(m.peak.v);
    const boxW = Math.max(24, text.length * 6.2 + 10);
    const boxH = 14;
    let box = chartPeakSideBox(m.x, m.y, boxW, boxH, pad, w, h, prefer);
    for (let i = 0; i < 6; i++) {
      const trial = { x0: box.boxX, x1: box.boxX + boxW, y0: box.boxY, y1: box.boxY + boxH };
      if (!occupied.some((o) => chartPeakBoxOverlap(trial, o, 4))) break;
      box = { ...box, boxY: Math.max(pad.t + 2, Math.min(h - pad.b - boxH - 2, box.boxY + (i % 2 ? -15 : 15))) };
    }
    occupied.push({ x0: box.boxX, x1: box.boxX + boxW, y0: box.boxY, y1: box.boxY + boxH });
    const lead = chartPeakLeadTarget(m.x, m.y, box.boxX, box.boxY, boxW, boxH);
    return renderPeakBadgeGroup(m, box.boxX, box.boxY, boxW, boxH, lead.x1, lead.y1, lead.x2, lead.y2);
  }

  function layoutPeakMarks(items, pad, w, h) {
    const list = (Array.isArray(items) ? items : []).filter((m) => m && Number.isFinite(m.x) && Number.isFinite(m.y));
    if (!list.length) return "";
    const occupied = [];
    const plotMid = (pad.l + w - pad.r) / 2;
    const sorted = [...list].sort((a, b) => a.x - b.x);
    return sorted
      .map((m, i) => {
        if (chartPeakNearTop(m.y, pad, h)) {
          return renderPeakFlagMark(m, pad, w, h, occupied);
        }
        let prefer = m.x < plotMid ? "right" : "left";
        if (list.length > 1) {
          prefer = m.kind === "viewers" ? "left" : "right";
          if (Math.abs(m.x - sorted[1 - i]?.x) > 120) {
            prefer = m.x < plotMid ? "right" : "left";
          }
        }
        return renderPeakSideMark(m, pad, w, h, prefer, occupied);
      })
      .join("");
  }
  ...
  function renderPeakMark(peak, x, y, extraClass, layout = {}, chartBox = null) {
    ...
    const m = { peak, x, y, extraClass: extraClass || "", kind: extraClass?.includes("chat") ? "chat" : "viewers" };
    if (layout.side) {
      return layoutPeakMarks([m], pad, w, h);
    }
    if (chartPeakNearTop(y, pad, h) && layout.labelX == null && layout.labelY == null) {
      return layoutPeakMarks([m], pad, w, h);
    }
    if (layout.labelX != null || layout.labelY != null) {
      const lx = Number(layout.labelX ?? x);
      const ly = Number(layout.labelY ?? y - 10);
      const text = fmtNum(v);
      const boxW = Math.max(24, text.length * 6.2 + 10);
      const boxH = 14;
      const badgeX = layout.anchor === "end" ? lx - boxW : layout.anchor === "start" ? lx : lx - boxW / 2;
      const badgeY = ly - 11;
      ...
      return `<g class="${cls.trim()}" pointer-events="none">
        ...
        <text x="${lx.toFixed(1)}" y="${ly.toFixed(1)}" text-anchor="${layout.anchor || "middle"}">${esc(text)}</text>
      </g>`;
    }
    return layoutPeakMarks([m], pad, w, h);
  }
  ...
  function renderDualPeakMarks(viewerPeak, chatPeak, xAtTime, yViewers, yChats, viewMinMs, viewMaxMs, pad, w, h) {
    ...
    if (!items.length) return "";
    return layoutPeakMarks(items, pad, w, h);
  }

  function scrollChartXIntoView(wrap, chartX) {
    if (chartIsFitZoom() || !wrap) return;
    const scroller = wrap.closest(".ending-dev-chart")?.querySelector?.("[data-chart-plot-scroll]");
    if (!scroller) return;
    const max = Math.max(0, scroller.scrollWidth - scroller.clientWidth);
    const target = Number(chartX) - scroller.clientWidth / 2;
    scroller.scrollLeft = Math.max(0, Math.min(max, target));
    scheduleChartPanFromPlotScroll(scroller);
  }
  ...
  function renderMetricChartPanel(points, opts) {
    ...
    const area = `${line} L${xAtTime(new Date(fullMaxMs).toISOString()).toFixed(1)},${(pad.t + innerH).toFixed(1)} L${xAtTime(new Date(fullMinMs).toISOString()).toFixed(1)},${(pad.t + innerH).toFixed(1)} Z`;
    const yTicks = [0, Math.round(maxV / 2), maxV];
    const yGrid = yTicks
      .map((v) => {
        const y = yAt(v).toFixed(1);
        return `<line class="ending-dev-chart__grid" x1="${pad.l}" y1="${y}" x2="${plotW - pad.r}" y2="${y}" />`;
      })
      .join("");
    const axisLeft = renderChartYAxisLeft(yTicks, yAt, h);
    const tickTimes = chartTimeTickTimes(fullMinMs, fullMaxMs, plotW - pad.l - pad.r);
    const xLabels = renderChartTimeAxisDual(tickTimes, xAtTime, pad, plotW, h);
    ...
    const plotSvg = `${renderChartClipDef(pad, plotW, h)}
          ${yGrid}
  ...
  function renderDualMetricChartPanel(viewerPoints, chatPoints, counts, replay) {
    ...
    const { xAtTime, yViewers, yChats } = chartDualTimeLayout(
      pad,
      plotW,
      h,
      fullMinMs,
    ...
    const yGrid = yTicksViewers
      .map((v) => {
        const y = yViewers(v).toFixed(1);
        return `<line class="ending-dev-chart__grid" x1="${pad.l}" y1="${y}" x2="${plotW - pad.r}" y2="${y}" />`;
      })
      .join("");
    const axisLeft = renderChartYAxisLeft(yTicksViewers, yViewers, h);
    const axisRight = renderChartYAxisRight(yTicksChats, yChats, h);
    const tickTimes = chartTimeTickTimes(fullMinMs, fullMaxMs, plotW - pad.l - pad.r);
    const xLabels = renderChartTimeAxisDual(tickTimes, xAtTime, pad, plotW, h);
    ...
    return {
      ...
      ${renderChartPlotStage({
        ...
        fullMinMs,
        fullMaxMs,
        wrapAttrs: `data-chart-dual="1"`,
        axisLeft,
        axisRight,
        ariaLabel: "시청자·채팅 화력 추이",
        plotSvg,
      })}
    </div>`,
      bind: {
  ...
  function mountDualMetricChartInteraction(root, opts) {
    ...
    bindDualInteractiveChart(wrap, {
      ...
      replay: opts.replay,
      width: Number(wrap.dataset.chartWidth) || opts.width || CHART_W,
      height: Number(wrap.dataset.chartHeight) || opts.height || CHART_H,
      pad,
      minMs: fullMinMs,
      maxMs: fullMaxMs,
      fullMinMs,
