...
def apply_runtime_config():
    global DATA_DIR, SCHEDULE_PATH, CALENDAR_ICS_PATH, MUSICBOOK_PATH, LINKS_PATH, PATCHNOTES_PATH, CONFIG_PATH, MANAGERS_PATH, SONG_REQUESTS_PATH
...
def _public_musicbook_payload(data):
    payload = json.loads(json.dumps(data if isinstance(data, dict) else {}))
    songs = payload.get("songs")
    if isinstance(songs, list):
        for song in songs:
            if isinstance(song, dict):
                song.pop("createdAt", None)
                song.pop("updatedAt", None)
    payload["apiScope"] = "public"
    return payload
...
def normalize_day(day_data):
    ...
    bangon = None
    if slots and not _is_off_day_slots(slots, fallback):
        bangon = normalize_bangon_time(day_data.get("bangonTime"))

    part2_raw = day_data.get("part2")
    if not slots and isinstance(part2_raw, dict):
        promoted = _normalize_slot_list(part2_raw.get("slots"), fallback)
        if promoted:
            slots = promoted
            if not bangon and not _is_off_day_slots(slots, fallback):
    ...
    # 2부: 1부 일정이 있을 때만 유효. 슬롯 또는 뱅온이 있으면 보존(편집 중 상태 포함)
    if slots and isinstance(part2_raw, dict):
        part2_slots = _normalize_slot_list(part2_raw.get("slots"), fallback)
        part2_bangon = None
        if not _is_off_day_slots(part2_slots, fallback):
            part2_bangon = normalize_bangon_time(part2_raw.get("bangonTime"))
        part2 = {}
        if part2_slots:
...
def _write_schedule_audit(action: str, detail: dict, *, month_id: str | None = None) -> None:
    ...
    try:
        ...
        meta = {
            ...
            "operationStats": detail.get("operationStats") or {},
            "operationLabels": [str(op.get("summary") or "") for op in operations[:10] if op.get("summary")],
        }
        if month_id:
            meta["monthId"] = month_id
        summary = schedule_audit.format_schedule_audit_summary(detail)
        audit_log("schedule", action, summary=summary, meta=meta)
    except Exception:
        pass
...
def content_revisions():
    return {
        ...
        "musicbook": _file_revision_token(MUSICBOOK_PATH),
...
def normalize_song(item, fallback_status="available"):
    ...
    out = {
        ...
        "tags": _normalize_song_tags(item.get("tags")),
...
def normalize_musicbook(data):
    ...
    tags_tax = taxonomy.get("tags")
    norm_tags_tax = []
    if isinstance(tags_tax, list):
        for tag in tags_tax:
            if isinstance(tag, dict):
                tag_id = str(tag.get("id", "")).strip()
                if tag_id:
                    norm_tags_tax.append(
                        {
                            "id": tag_id,
                            "label": str(tag.get("label") or tag_id).strip() or tag_id,
                            "group": str(tag.get("group") or "").strip(),
                        }
                    )
            elif str(tag).strip():
                tid = str(tag).strip()
                norm_tags_tax.append({"id": tid, "label": tid, "group": ""})
...
def inject_ga4_snippet(html: str) -> str:
    if "googletagmanager.com/gtag/js" in html:
        return html
    mid = ga4_analytics.measurement_id()
    if not mid or not re.fullmatch(r"G-[A-Z0-9]+", str(mid)):
        return html
    safe_id_attr = escape(str(mid), quote=True)
    safe_id_js = json.dumps(str(mid))
    block = f"""  <script async src="https://www.googletagmanager.com/gtag/js?id={safe_id_attr}"></script>
  <script>
    window.dataLayer = window.dataLayer || [];
    function gtag(){{dataLayer.push(arguments);}}
    gtag("js", new Date());
    gtag("config", {safe_id_js});
  </script>
"""
    return html.replace("</head>", block + "</head>", 1)
...
def api_ops_audit_log():
    ...
    for i in range(1, max(1, AUDIT_LOG_BACKUPS) + 1):
        paths.append(AUDIT_LOG_PATH.with_suffix(f".jsonl.{i}"))
        ...
        for p in paths:
            ...
            for line in text.splitlines():
                ...
                if email:
                    ...
                    if not prev or (name and not prev.get("name")):
                        actors_map[email] = {"email": email, "name": name}
                if actor:
...
def api_ops_audit_detail(detail_id):
    ...
    return jsonify({"ok": True, "detailId": detail_id, "detail": detail})
...
def api_auth_load_test_login():
    ...
    audit_log(
        ...
        "login",
        summary=f"로그인 · {name}",
        meta={"email": email, "provider": "load_test"},
    )
    return jsonify(auth_payload())
...
def api_auth_logout():
    user = current_user()
    if user or is_dev_streamer():
        email = normalize_manager_email(user.get("email", "")) if user else ""
        name = str(user.get("name") or "").strip() if user else ""
        label = name or email or ("스트리머 모드" if is_dev_streamer() else "게스트")
        audit_log(
            "auth",
            "logout",
            summary=f"로그아웃 · {label}",
...
def api_schedule_put():
    ...
    try:
        ...
        pass
    return jsonify({"ok": True, "updatedAt": utc_now()})
...
def api_musicbook_get():
    data = load_musicbook()
    if request_can_edit():
        return jsonify({**data, "updatedAt": utc_now(), "apiScope": "editor"})
    return jsonify({**_public_musicbook_payload(data), "updatedAt": utc_now()})
...
def api_musicbook_put():
    ...
    try:
        old_songs = old_data.get("songs") if isinstance(old_data.get("songs"), list) else []
        new_songs = data.get("songs") if isinstance(data.get("songs"), list) else []
        meta = {
            "songsBefore": len(old_songs),
            "songsAfter": len(new_songs),
            "renames": renames[:50] if isinstance(renames, list) else [],
        }
        audit_log("musicbook", "put", summary=f"노래책 저장 · songs {len(old_songs)}→{len(new_songs)}", meta=meta)
    except Exception:
        pass
...
def api_song_requests_like_toggle(req_id):
    ...
    audit_song_request(
        like_action,
        summary=f"{like_label} · {_song_request_audit_label(updated)} ({new_count})",
        req=updated,
